...
(function () {
  var NAV_GROUPS = {
    ...
    S24: 'settings', S25: 'settings', S43: 'sysset', S44: 'people', S45: 'audit',
    S26: 'staff',
    S42: 'inbox',
    S39: 'money', S40: 'money',
    S41: 'announce'
  ...
  var items = [
    ...
    // Not admin-only: staff answer "has this person rented here before?" at the desk.
    { key: 'people', href: 's44-tenant-directory.html', icon: '☰', label: 'ทะเบียนผู้เช่า' },
    // Lands on กำไร (S39); the full book is one click further, on S47.
    { key: 'money', href: 's39-profit.html', icon: '฿', label: 'กำไร', adminOnly: true },
    ...
    // Separate from ห้องและราคา on purpose: prices are a daily operational edit,
    // these are the rules of the business and change once a year.
    { key: 'sysset', href: 's43-system-settings.html', icon: '⚙', label: 'ตั้งค่าระบบ', adminOnly: true },
    { key: 'audit', href: 's45-audit-log.html', icon: '⏱', label: 'ประวัติการใช้งาน', adminOnly: true }
  ];
  ...
  topbar.innerHTML =
    ...
    // No search box up here. It used to sit on every page, where an unlabelled
    // box next to a bill or a meter sheet reads as "filter this page" and instead
    // navigates away (owner, 2026-08-01). Lookup now lives in one labelled place
    // on the dashboard — see mountSearch below.
  ...
  // ---- Lookup, mounted on request ----
  // A page that wants the room/tenant/invoice lookup calls AmanewNav.mountSearch(el)
  // and owns the label around it, so the box always says what it searches.
  function mountSearch(host) {
    if (!host || !data || !data.searchAll) return;
    host.classList.add('topbar-search', 'page-search');
    host.innerHTML =
      '<span class="ts-ic">🔍</span>' +
      '<input type="search" id="global-search" placeholder="พิมพ์ เลขห้อง / ชื่อผู้เช่า / เบอร์โทร / เลขบิล" autocomplete="off">' +
      '<div class="ts-results" id="ts-results"></div>';
    var searchInput = host.querySelector('#global-search');
    var resultsBox = host.querySelector('#ts-results');
    var cursor = -1;
  ...
  window.AmanewNav = { mountSearch: mountSearch };
